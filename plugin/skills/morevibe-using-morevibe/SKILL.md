...
4. Use `morevibe-session-brief` first when the session needs a fast state restore.
5. Use `morevibe-query-harness` first when the task needs quick retrieval of current memory.
6. Read `.morevibe/wiki/state.md` and `.morevibe/wiki/index.md`.
7. Read the relevant canon documents for the current task.
...
- Use `morevibe-session-brief` when a quick structured startup summary will reduce re-reading.
- Use `morevibe-orchestrate-subagents` when the task should follow an orchestrator-first delegation model.
