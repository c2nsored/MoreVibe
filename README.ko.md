...
- 지원되는 도구용 프로젝트 로컬 Skill 폴더
- 역할 분담형 오케스트레이션을 지원하는 도구용 프로젝트 로컬 에이전트/라우팅 파일
- 재사용 가능한 규칙 및 운영 지침
...
새롭거나 비어 있는 프로젝트라면 시작용 `AGENTS.md`, 기본 MoreVibe Skill 세트, 그리고 지원되는 도구의 프로젝트 로컬 역할 파일까지 함께 준비할 수 있습니다.
...
### Codex
Codex에 특화된 엔트리 가이드라인을 지원하며, 플러그인/헬퍼 구조와 함께 프로젝트 로컬 `.codex/` 역할 템플릿도 지원합니다.
...
* 지원 도구용 기본 프로젝트 Skill 설치
* Codex용 프로젝트 로컬 역할 템플릿
* Windows GUI 인스톨러 지원
...
│  ├─ canon/
│  └─ wiki/
├─ .codex/             # Codex 프로젝트 통합 시
├─ .claude/            # Claude Code 프로젝트 통합 시
├─ AGENTS.md / CLAUDE.md / GEMINI.md
└─ ...your project files...
```

설치 대상 도구에 따라 아래도 함께 생길 수 있습니다:

- 루트 `AGENTS.md`가 없으면 시작 진입점으로 자동 생성
- `.agents/skills/`에 기본 MoreVibe workflow skills 설치
- `.claude/skills/`, `.claude/agents/` 설치
- `.codex/config.toml`, `.codex/agents/*.toml` 설치
...
설치가 끝나면 인스톨러가 간단한 bootstrap health 리포트도 출력하므로, 진입점/Skill/에이전트가 실제로 잘 생성됐는지 바로 확인할 수 있습니다.
